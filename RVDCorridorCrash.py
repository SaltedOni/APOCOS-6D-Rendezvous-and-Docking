...
INJECT_CORRIDOR_VIOLATION = True
...
#  METRICS FILE
# ==========================================================
METRICS_FILE = "relative_metrics_corrected.json"
...
class LiftOffWithTiltRecovery:
    ...
    @staticmethod
    def move_to_xy_guarded(xbot_id, x, y, fsm: DockingFSM, target: Pose2D, speed=None):
        if speed is None:
            speed = XY_SPEED_FULL
        ...
        while True:
            ...
            if st.xbot_state == pm.XBOTSTATE.XBOT_IDLE:
                break

            cur_x, cur_y = (
                float(st.feedback_position_si[0]),
                float(st.feedback_position_si[1]),
            )
            ee = Pose2D(cur_x, cur_y)

            if not fsm.check_guards(target, ee):
                print("🛑 Guard violation DURING motion — stopping mover.")
                bot.stop_motion(0)
                return False

            time.sleep(0.03)  # ~30 Hz guard rate
    ...
    def _phase_mid_approach(xbot_id, fsm: DockingFSM, target: Pose2D):
        ...
        # --- INTENTIONAL corridor violation injection ---
        if INJECT_CORRIDOR_VIOLATION:
            # 6 mm lateral offset at ~47 mm range ≈ 7.3°
            # well outside tightened cone downstream
            mid_x, mid_y = resolve(target, MID_OFFSET_X, -0.100)
        else:
        ...
        ok = LiftOffWithTiltRecovery.move_to_xy_guarded(
            xbot_id,
            mid_x,
            mid_y,
            fsm,
    ...
    def run_liftoff_routine():
        ...
        # Capture final lateral alignment error at HOLD exit.
        # base_y is the actual achieved Y after any fine correction,
        # so this is the best proxy for positional accuracy at contact.
        metrics.final_lateral_err_m = abs(base_y - target_pose.y)
        ...
        while True:
            ...
            print("❗ Collision or tilt → Recovering Z and retrying in place…")
            LiftOffWithTiltRecovery.recover(xbot_id)
            attempt += 1
...
def run_simulation_and_viz():
    ...
    waypoints = [
        ...
        (*R(target, CORRIDOR_ENTRY_OFFSET_X, 0.006), DockState.CORRIDOR_ENTRY),
